...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[..\_Server\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[..\_Server\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[..\_Server\apache\include];
my ${exec_prefix} = q[..\_Server\apache];
my ${datadir} = q[..\_Server\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[..\_Server\apache\lib];
