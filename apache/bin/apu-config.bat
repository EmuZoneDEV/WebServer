...
goto endofperl
@rem ';
#!\AC Web Ultimate Repack\Server\perl\bin\perl.exe
#line 15
use strict;
use warnings;
use Getopt::Long;
...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[\AC Web Ultimate Repack\Server\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[\AC Web Ultimate Repack\Server\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[\AC Web Ultimate Repack\Server\apache\include];
my ${exec_prefix} = q[\AC Web Ultimate Repack\Server\apache];
my ${datadir} = q[\AC Web Ultimate Repack\Server\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[\AC Web Ultimate Repack\Server\apache\lib];
...
my ${prefix} = q[\AC Web Ultimate Repack\Server\apache];
